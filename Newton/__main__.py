...
import sys
import traceback
import Newton.modules.sql.users_sql as sql
...
from Newton.modules import ALL_MODULES
from Newton.modules.helper_funcs.chat_status import is_user_admin
from Newton.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("Newton.modules." + module_name)
...
def shukurenai_about_callback(update, context):
    ...
    if query.data == "shukurenai_":
        query.message.edit_text(
            ...
            "\n乂 I can restrict users."
            "\n乂 I can greet users with customizable welcome messages and even set a group's rules."
            "\n乂 I have an advanced anti-flood system."
            "\n乂 I can warn users until they reach max warns, with each predefined actions such as ban, mute, kick, etc."
            "\n乂 I have a note keeping system, blacklists, and even predetermined replies on certain keywords."
            "\n乂 I check for admins' permissions before executing any command and more stuffs"
            "\n\n_Newton's licensed under the GNU General Public License v3.0_"
            "\n\n Click on button bellow to get basic help for Newton."
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
