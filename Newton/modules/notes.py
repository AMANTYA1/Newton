...
from Newton import dispatcher, MESSAGE_DUMP, LOGGER
from Newton.modules.disable import DisableAbleCommandHandler
from Newton.modules.helper_funcs.chat_status import user_admin, user_can_change
	...
	if note.is_reply:
		...
		try:
			...
			if excp.message == "Entity_mention_user_invalid":
				...
				message.reply_text("This note could not be sent, as it is incorrectly formatted, Please ask in @zonerschat if you can't figure out why!")
...
def rmallbutton(update, context):
...
def __chat_settings__(chat_id, user_id):
	...
	return "There is a note `{}` in this chat.".format(len(notes))
...
REMOVEALL_HANDLER = CommandHandler("rmall", rm_all, filters=Filters.chat_type.groups, run_async=True)

RMBTN_HANDLER = CallbackQueryHandler(rmallbutton, pattern=r"rmall_", run_async=True)
